...
export default function DiscoveryWizardPage() {
  ...
  const validateStep = (currentStep: number) => {
    switch (currentStep) {
      ...
      case 2: // Section B
        return (
          formData.customerChannels.length > 0 &&
          (!formData.customerChannels.includes("Other") ||
            !!formData.otherCustomerChannel) &&
          !!formData.conversionMechanism &&
          (formData.conversionMechanism !== "Other" ||
            !!formData.otherConversionMechanism) &&
          formData.workTriggers.length > 0 &&
          (!formData.workTriggers.includes("Other") ||
            !!formData.otherWorkTrigger) &&
          !!formData.revenueTracking &&
          (formData.revenueTracking !== "Other" ||
            !!formData.otherRevenueTracking) &&
          !!formData.revenuePredictability &&
          !!formData.scalingImpact &&
          (formData.scalingImpact !== "Other" ||
            !!formData.otherScalingImpact) &&
          formData.handoffPoints.length > 0 &&
          (!formData.handoffPoints.includes("Other") ||
            !!formData.otherHandoffPoint)
        );
      case 3: // Section C
        const coreToolsValid =
          formData.coreTools.length > 0 &&
          (!formData.coreTools.includes("Other") || !!formData.otherCoreTool) &&
          formData.coreTools.every(
            (t) => t === "Other" || !!(formData.coreToolNames[t] || "").trim(),
          );
        return (
          coreToolsValid &&
          !!formData.sourceOfTruth &&
          (formData.sourceOfTruth !== "Other" ||
            !!formData.otherSourceOfTruth) &&
          !!formData.automationStatus
        );
      case 4: // Section D
        return (
          formData.humanJudgmentLevel.length > 0 &&
          formData.manualTimeDrains.length > 0 &&
          (!formData.manualTimeDrains.includes("Other") ||
            !!formData.otherManualTimeDrain) &&
          formData.errorProneTasks.length > 0 &&
          (!formData.errorProneTasks.includes("Other") ||
            !!formData.otherErrorProneTask) &&
          formData.delayPoints.length > 0 &&
          (!formData.delayPoints.includes("Other") ||
            !!formData.otherDelayPoint) &&
          !!formData.operationsVisibility &&
          (formData.operationsVisibility !== "Other" ||
            !!formData.otherOperationsVisibility)
        );
      case 5: // Section E (systemAccess/Q18 removed)
        return (
          formData.complianceConstraints.length > 0 &&
          (!formData.complianceConstraints.includes("Other") ||
            !!formData.otherComplianceConstraint) &&
          formData.environments.length > 0 &&
          (!formData.environments.includes("Other") ||
            !!formData.otherEnvironment) &&
          !!formData.vendorConstraints &&
          (formData.vendorConstraints !== "Yes" ||
            !!formData.vendorConstraintDetails)
        );
      case 6: // Section F
        return (
          !!formData.implementationTimeline &&
          (formData.implementationTimeline !== "Other" ||
            !!formData.otherImplementationTimeline) &&
          formData.successMetrics.length > 0 &&
          (!formData.successMetrics.includes("Other") ||
            !!formData.otherSuccessMetric) &&
          !!formData.primaryOutcome &&
          (formData.primaryOutcome !== "Other" ||
            !!formData.otherPrimaryOutcome) &&
          !!formData.inactionConsequence &&
          (formData.inactionConsequence !== "Other" ||
            !!formData.otherInactionConsequence) &&
          formData.proposalCriteria.length > 0 &&
          (!formData.proposalCriteria.includes("Other") ||
            !!formData.otherProposalCriteria)
        );
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h1 className="text-xl font-bold">
                {DISCOVERY_SCAN_COPY.headline.replace("\n", " ")}
              </h1>
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-secondary/40 rounded-xl p-5 border border-border">
              {isFreeScansLoading ? (
                ...
                <>
                  <div className="text-2xl font-bold text-emerald-500 mb-0.5">
                    FREE
                  </div>
              ...
              <ul className="space-y-1.5 text-xs text-muted-foreground">
                <li className="flex items-center gap-1.5">
                  ...
                  {DISCOVERY_SCAN_COPY.footer.replace(
    ...
    <div className="animate-in fade-in slide-in-from-right-8 duration-300">
      <div className="flex items-center gap-3 mb-6 pb-6 border-b border-border">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            How money and work flow through your business.
          </p>
        </div>
      </div>
    ...
    <div className="animate-in fade-in slide-in-from-right-8 duration-300">
      <div className="flex items-center gap-3 mb-6 pb-6 border-b border-border">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Identify where friction exists in your day-to-day.
          </p>
        </div>
      </div>

      <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-4 mb-8 text-sm text-amber-600">
        You don’t need to identify bottlenecks. Just answer based on your
        day-to-day reality.
      </div>
    ...
    <div className="animate-in fade-in slide-in-from-right-8 duration-300">
      ...
      <div className="space-y-3">
        <label className="block text-lg font-bold text-foreground">
          27. Is there anything about your business that would change how
        ...
        <div className="w-full max-w-2xl bg-card border border-border rounded-2xl p-6 md:p-10 shadow-xl relative min-h-[400px] flex flex-col">
          ...
          <div className="pt-4 mt-4 border-t border-border flex justify-between items-center">
            ...
            <button
              ...
              className="bg-primary hover:bg-primary/90 text-primary-foreground px-8 py-3 rounded-xl font-bold flex items-center gap-2 transition-all shadow-lg shadow-primary/20 disabled:opacity-70"
            >
              {isFinishing ? (
                <>
