...
export default async function ThreadPage({ params }: PageProps) {
  ...
  if (isBuyer) {
    otherPartyName =
      conversation.seller.displayName || conversation.seller.legalFullName;
  ...
  if (conversation.order) {
    const rawMs =
      (conversation.order.milestones as Record<string, unknown>[]) || [];
    orderContext = {
    ...
    solution: conversation.solution
      ? ({
          ...
          implementation_price:
            conversation.solution.implementationPriceCents / 100,
          monthly_cost_min: conversation.solution.monthlyCostMinCents
            ? conversation.solution.monthlyCostMinCents / 100
            : 0,
          monthly_cost_max: conversation.solution.monthlyCostMaxCents
            ? conversation.solution.monthlyCostMaxCents / 100
            : 0,
          delivery_days: conversation.solution.deliveryDays,
          status: conversation.solution
            .status as import("@/types").SolutionStatus,
          integrations: conversation.solution.integrations,
          implementation_price_cents:
            conversation.solution.implementationPriceCents,
      ...
      <main className="flex-1 overflow-y-auto h-[calc(100vh-4rem)]">
        <div className="container mx-auto px-4 py-6 max-w-6xl">
          <ThreadView
            ...
            expertCalendarUrl={
              isBuyer
                ? (conversation.seller.calendarUrl ?? undefined)
                : undefined
            }
            meetLinkUrl={
              isSeller
