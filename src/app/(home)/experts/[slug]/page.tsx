...
export async function generateMetadata({ params }: PageProps) {
  ...
  const description =
    expert.bio ||
    `Hire ${expert.displayName} for AI automations on ${BRAND_NAME}.`;
  return {
    ...
    return (
      <div className="container mx-auto px-4 py-20 text-center">
        ...
        <p className="text-muted-foreground mb-8">
          The expert you are looking for does not exist or has not been approved
          yet.
        </p>
        <Link
          href="/solutions"
    ...
    <div className="min-h-screen pb-20">
      ...
      <div className="bg-secondary/10 border-b border-border py-12">
        <div className="container mx-auto px-4">
          <Link
            href="/solutions"
            className="text-sm text-muted-foreground hover:text-foreground flex items-center mb-8 w-fit"
          >
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-2 mb-8">
                {expert.tools.map((tool) => (
                  <span
                    key={tool}
                    className="px-3 py-1 bg-card border border-border rounded-md text-sm font-medium"
                  >
                    {tool}
                  </span>
                ))}
      ...
      <div className="container mx-auto px-4 py-16" id="solutions">
        <div className="flex items-center justify-between mb-8">
          ...
          <span className="text-muted-foreground">
            {expertSolutions.length} results
          </span>
        </div>
      ...
      <div className="border-t border-border bg-secondary/5 py-12">
        <div className="container mx-auto px-4 text-center max-w-2xl">
          <h3 className="text-xl font-bold mb-2">
            Work directly with the specialist who delivers the implementation
          </h3>
          <p className="text-muted-foreground">
            Clear scope. Real delivery. One point of contact from request to
            completion.
          </p>
        </div>
      </div>
