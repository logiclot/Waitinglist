...
export interface SolutionPublishData {
  ...
  milestones?: { title?: string; price?: number; description?: string }[];
}
...
export function validateSolutionForPublish(solution: SolutionPublishData): ValidationResult {
  ...
  if (milestones.length === 0) {
    ...
    const milestoneTotalCents = Math.round(
      milestones.reduce((sum, m) => sum + (m.price ?? 0), 0) * 100
    );
...
export function milestoneTotalCents(
  milestones: { price?: number }[]
): number {
  return Math.round(milestones.reduce((sum, m) => sum + (m.price ?? 0), 0) * 100);
}
