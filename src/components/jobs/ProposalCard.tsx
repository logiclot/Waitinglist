...
function EditProposalForm({
  ...
  const [price, setPrice] = useState(
    bid.priceEstimate?.replace(/[^0-9.]/g, "") || "",
  );
  const [timeline, setTimeline] = useState(bid.estimatedTime || "");
  const [approach, setApproach] = useState(bid.proposedApproach || "");
  const [saving, setSaving] = useState(false);
  const input =
    "w-full bg-background border border-border rounded-md px-3 py-2 text-sm focus:ring-1 focus:ring-primary focus:border-primary outline-none transition";
    ...
    <div className="border-t border-border bg-secondary/5 p-5 space-y-4">
      ...
      {proposal && (
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">
            Full proposal JSON (advanced — edit with care)
          </label>
          <textarea
      ...
      <div className="flex gap-2 justify-end pt-1">
        <button
          onClick={onClose}
          type="button"
          className="px-3 py-1.5 rounded-lg text-sm text-muted-foreground hover:text-foreground border border-border"
        >
          Cancel
        </button>
        <button
...
export function ProposalCard({
  ...
  const milestones =
    (bid.order?.milestones as Record<string, unknown>[] | undefined) ?? [];
  ...
  const canEdit =
    isExpertView && bid.status !== "accepted" && new Date() < editableUntil;
  ...
  const tick = useCallback(() => {
    ...
    if (remaining > 0 && remaining <= WARN_THRESHOLD_MS && !warnFired) {
      setWarnFired(true);
      toast.warning("5 minutes left to edit your proposal — it locks soon.", {
        duration: 8000,
      });
    }
  ...
  const displayName = isPostTenderView
    ? (anonymizedLabel ?? "Expert")
  ...
  return (
    ...
    >
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            ...
            {proposal?.automationTitle && (
              <h4 className="font-semibold text-base mt-3 mb-1">
                {proposal.automationTitle}
              </h4>
            )}
                ...
                {isExpertView && !isAwarded && (
                  <div className="flex items-center gap-2">
                    {canEdit ? (
                      <>
                        <span
                          className={`text-xs flex items-center gap-1 ${msRemaining < WARN_THRESHOLD_MS ? "text-amber-500 font-semibold" : "text-muted-foreground"}`}
                        >
      ...
      {!editing && expanded && proposal && (
        <div
          key={refreshKey}
          className="border-t border-border bg-secondary/5 p-5 space-y-5"
        >
          {/* Problem addressed */}
          ...
          {proposal.outcomes && proposal.outcomes.length > 0 && (
            <div>
              <div className="flex items-center gap-2 text-sm font-semibold mb-3">
                <CheckCircle2 className="h-4 w-4 text-green-500" /> Measurable
                outcomes
              </div>
          ...
          {proposal.phases && proposal.phases.length > 0 && (
            <div>
              <div className="flex items-center gap-2 text-sm font-semibold mb-3">
                <LayoutList className="h-4 w-4 text-primary" /> Implementation
                plan
              </div>
    ...
    return (
      <div className="text-center py-12 border border-dashed border-border rounded-xl">
        ...
        <p className="text-muted-foreground text-sm max-w-sm mx-auto">
          Qualified experts will submit structured proposals here. Typically
          within 48 hours.
        </p>
      </div>
    );
    ...
    <div className="space-y-4">
      {isPostTenderView && (
        <div className="flex items-center gap-2 bg-secondary/30 border border-border rounded-xl px-4 py-3">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            ...
            — See how other experts approached this brief. Names are anonymized.
            Use this to sharpen your future proposals.
          </p>
        </div>
      )}
      ...
      {bids.map((bid, index) => (
        <ProposalCard
          ...
          isOwnBid={
            isPostTenderView &&
            !!(expertSpecialistId && bid.specialistId === expertSpecialistId)
          }
          anonymizedLabel={
            isPostTenderView
              ? `Expert ${labels[index] ?? index + 1}`
              : undefined
          }
        />
      ))}
