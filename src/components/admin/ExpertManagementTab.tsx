...
function FeeEditor({ current, onSave }: { current: number; onSave: (v: number) => void }) {
  const [editing, setEditing] = useState(false);
  const [val, setVal] = useState(String(current));

  if (!editing) {
    return (
      <button onClick={() => setEditing(true)} className="text-sm font-bold hover:underline tabular-nums">
        {current}%
      </button>
    );
  }
  return (
    <span className="flex items-center gap-1">
      ...
      />
      <button
        onClick={() => { onSave(parseInt(val)); setEditing(false); }}
              ...
              return [
                ...
                >
                  ...
                  <td className="px-6 py-4 align-top">
                    ...
                    <div className="text-xs text-muted-foreground">{expert.user?.email}</div>
                    {expert.country && (
                  ...
                  <td className="px-6 py-4 align-top">
                    <div className="flex items-center gap-1.5 text-xs mb-1">
                      <Briefcase className="w-3 h-3 text-muted-foreground" /> {expert.yearsExperience} yrs
                    </div>
                    <div className="flex flex-wrap gap-1">
                      ...
                      {(expert.tools?.length ?? 0) > 3 && <span className="text-[10px] text-muted-foreground">+{(expert.tools?.length ?? 0) - 3}</span>}
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 align-top">
                    <div className="flex flex-col gap-1.5">
                      {expert.status === "PENDING_REVIEW" && <span className="inline-flex w-fit px-2 py-0.5 rounded bg-yellow-500/10 text-yellow-500 text-xs font-medium border border-yellow-500/20">Pending</span>}
                      {expert.status === "APPROVED" && <span className="inline-flex w-fit px-2 py-0.5 rounded bg-green-500/10 text-green-500 text-xs font-medium border border-green-500/20">Approved</span>}
                      {/* Bid quality score */}
                      {(expert.bidQuality?.up || expert.bidQuality?.down) ? (
                        <div className="flex items-center gap-1.5 text-xs">
                          <ThumbsUp className="h-3 w-3 text-primary" />
                          <span className="font-medium">{expert.bidQuality.up}</span>
                          <ThumbsDown className="h-3 w-3 text-destructive ml-1" />
                          <span className={`font-medium ${expert.bidQuality.down >= 3 ? "text-destructive" : ""}`}>{expert.bidQuality.down}</span>
                        </div>
                      ) : null}
                      {/* Bid ban indicator */}
                  ...
                  {!isPending && (
                    <td className="px-6 py-4 align-top" onClick={(e) => e.stopPropagation()}>
                      <div className="space-y-2">
                        ...
                        <select
                          value={tier}
                          onChange={(e) => onSetTier(expert.id, e.target.value as "STANDARD" | "PROVEN" | "ELITE")}
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      {expert.status === "PENDING_REVIEW" && (
                        <div className="flex gap-2">
                          <button onClick={() => onApprove(expert.id)} className="px-3 py-1 bg-green-500 text-black font-bold rounded text-xs hover:bg-green-400">Approve</button>
                          <button onClick={() => onSuspend(expert.id)} className="px-3 py-1 border border-border rounded text-xs hover:bg-secondary">Reject</button>
                        </div>
                      )}
                      ...
                      {!expert.isFoundingExpert && expert.status === "APPROVED" && (
                        <button onClick={() => onMakeFounding(expert.id)} className="text-xs px-3 py-1 rounded border border-yellow-500/30 text-yellow-500 hover:bg-yellow-500/10">
                          ★ Make Founding
                        </button>
                      )}
                      ...
                      {expert.user?.id && (
                        <button onClick={() => onDeleteUser(expert.user.id)} className="text-muted-foreground hover:text-destructive transition-colors mt-1" title="Delete user account">
                ...
                // Expanded detail row
                isExpanded && (
                  <tr key={`${expert.id}-details`} className="bg-secondary/10" onClick={(e) => e.stopPropagation()}>
                    <td colSpan={isPending ? 4 : 5} className="px-6 py-5">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm animate-in slide-in-from-top-1 duration-150">
                        <div className="space-y-2">
                          <h4 className="text-xs font-bold uppercase text-muted-foreground tracking-wider">Professional</h4>
                          <div className="space-y-1 text-muted-foreground">
                            <div><span className="text-foreground">Legal Name:</span> {expert.legalFullName}</div>
                            <div><span className="text-foreground">Location:</span> {expert.country}</div>
                            <div><span className="text-foreground">Experience:</span> {expert.yearsExperience}</div>
                            <div><span className="text-foreground">Tools:</span> {expert.tools?.join(", ") || "—"}</div>
                            <div><span className="text-foreground">Stripe:</span> {expert.stripeAccountId || <span className="text-red-400">Not connected</span>}</div>
                            <div><span className="text-foreground">Calendar:</span> {expert.calendarUrl ? <a href={expert.calendarUrl} className="text-blue-400 underline" target="_blank" rel="noreferrer">View</a> : <span className="text-muted-foreground">—</span>}</div>
                          </div>
                        </div>
                        <div className="space-y-2">
                          <h4 className="text-xs font-bold uppercase text-muted-foreground tracking-wider">Private Notes</h4>
                          <div className="text-xs bg-background p-3 rounded border border-border font-mono whitespace-pre-wrap text-muted-foreground">
                            {expert.bio || "No notes."}
                          </div>
                        </div>
                        <div className="space-y-2">
                          <h4 className="text-xs font-bold uppercase text-muted-foreground tracking-wider">Portfolio</h4>
                          <div className="flex flex-col gap-1">
                            {(Array.isArray(expert.portfolioLinks) ? expert.portfolioLinks : []).map((link: string, i: number) => (
                              <a key={i} href={link} target="_blank" rel="noreferrer" className="text-xs text-blue-400 hover:underline flex items-center gap-1">
                                <ExternalLink className="w-3 h-3" /> {link}
                              </a>
                            ))}
                            {!(Array.isArray(expert.portfolioLinks) && expert.portfolioLinks.length > 0) && <span className="text-xs text-muted-foreground">No links.</span>}
                          </div>
                        </div>
                      </div>
                    </td>
